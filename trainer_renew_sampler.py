...
class my_Branin:
    ...
    def predict(self, x):
        ...
        return y1 + lambda_my_xunhuan * y2    # shape (batch_size,)
...
# -----------------------------
    ...
    if sys.platform.startswith("win"):
        src_dir = os.path.join(expt_save_path, f"wandb/latest-run/files/results/{run_specific_str}/")
...
def run_evaluate_test(
    ...
    all_designs = np.concatenate([init_X, designs], axis=0)
    all_results = np.concatenate([init_Y_new, results], axis=0)
    ...
    # merged_dataset_path = "design_baselines/diff_branin/dataset/branin_gaussian_5k.p"
    merged_dataset_path = "design_baselines/diff_branin/dataset/branin_gaussian_multi_region_5k.p"
    with open(merged_dataset_path, "wb") as f:
        pkl.dump((all_designs, all_results), f)

    print("✅ 已更新数据集:", merged_dataset_path)
